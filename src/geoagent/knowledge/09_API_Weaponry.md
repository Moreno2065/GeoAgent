...
> **【作用域声明】高德地图 API 仅限以下两类绝对特定场景使用：**
>
> - **🅰️ 地址翻译**：用户说了中文地名（如「天安门」）→ 调用 `geocode` 返回经纬度；用户提供坐标 → 调用 `regeocode` 返回地址描述。
> - **🅱️ 路径导航**：用户要求计算国内 A点到B点的驾车/步行/公交路线 → 调用 `direction_routing` 系列。
>
> **🚨 严禁使用高德 API 进行任何几何图形计算！** 禁止行为包括但不限于：画缓冲区、算面积、做叠置分析、交集/并集/差集。几何计算必须交给 **GeoPandas / Shapely** 完成，结果由 **Folium (Leaflet)** 渲染。
