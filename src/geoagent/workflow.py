...
def planner_node(state: WorkflowState) -> WorkflowState:
    ...
    event_cb = state.get("_event_callback")
    ...
    # ── 动态 System Prompt（根据任务类型选择对应 One-Shot 示例） ────────────
    # 强制剥夺说话权：绝对禁令（Terminal Constraints）
    base_system = """你是一个严格的 GIS 规划助手，代号 PLANBOT。
...
5. 你的输出必须是一个**纯粹的 JSON 数组**，首字符必须是 `[`。
6. 必须严格使用下方可用工具列表中的精确 `tool_name`，禁止幻想不存在的工具。
...
### 示例输出格式（唯一合法输出）
[{"step_id":1,"action_name":"osmnx_routing","input_files":[],"output_file":"workspace/route.html","description":"计算路径","depends_on":[],"expected_output":"路径结果"}]
"""
...
输出：
[{"step_id":1,"action_name":"osmnx_routing","input_files":[],"output_file":"workspace/route.html","description":"计算天安门到北京站的步行最短路径","depends_on":[],"expected_output":"步行路径和交互地图"}]
"""
...
输入：在深圳市进行多准则选址分析
输出：
[{"step_id":1,"action_name":"multi_criteria_site_selection","input_files":[],"output_file":"workspace/site.html","description":"基于MCDA多准则决策进行智能选址","depends_on":[],"expected_output":"选址结果和可视化地图"}]
"""
    elif any(kw in query_lower for kw in ["ndvi", "植被", "raster", "波段", "landsat", "sentinel"]):
        examples = """
## 【One-Shot 参考】
...
输入：计算北京市NDVI植被指数
输出：
[{"step_id":1,"action_name":"get_raster_metadata","input_files":["data/satellite.tif"],"output_file":null,"description":"获取遥感影像元数据","depends_on":[],"expected_output":"影像元数据"},{"step_id":2,"action_name":"calculate_raster_index","input_files":["data/satellite.tif"],"output_file":"workspace/ndvi.tif","description":"计算NDVI","depends_on":[1],"expected_output":"NDVI结果栅格"}]
"""
...
输入：生成上海市建筑物3D可视化地图
输出：
[{"step_id":1,"action_name":"render_3d_map","input_files":["data/buildings.shp"],"output_file":"workspace/3d_map.html","description":"生成建筑物3D可视化","depends_on":[],"expected_output":"PyDeck 3D交互地图"}]
"""
    else:
        examples = """
## 【One-Shot 参考】
...
输入：完成GIS分析任务
输出：
[{"step_id":1,"action_name":"osmnx_routing","input_files":[],"output_file":"workspace/route.html","description":"执行分析","depends_on":[],"expected_output":"分析结果"}]
"""
    ...
    try:
        client: OpenAI = state["_client"]
        model_name = state.get("_model", "deepseek-chat")

        # 尝试使用 JSON Mode（如果模型支持）
        extra_kwargs = {}
        if "deepseek" in model_name.lower():
            # DeepSeek 支持 response_format 参数
        ...
        if not plan:
            ...
            strict_system = (
                ...
                "绝对禁止 ```json 或 ``` 围栏。绝对禁止任何文字。只能输出一个 JSON 数组。"
                "示例：`[{\"step_id\":1,\"action_name\":\"osmnx_routing\",\"input_files\":[],\"output_file\":\"workspace/route.html\",\"description\":\"计算路径\",\"depends_on\":[],\"expected_output\":\"路径结果\"}]`"
            )
            strict_user = (
                f"只输出 JSON 数组。禁止任何其他文字。\n"
                f"用户任务：{user_query}\n"
        ...
        print(f"[planner_node] 计划生成异常 -> {e}")
        _emit(state, "plan_failed", {"msg": f"计划生成异常: {e}"})
        state["plan"] = []
        state["mode"] = "react"
        state["error"] = str(e)
        state["is_complete"] = False
...
def reviewer_node(state: WorkflowState) -> WorkflowState:
    ...
    # 获取当前步骤结果（最后一个）
    if not step_results:
        # 如果没有结果记录，说明 executor_node 跳过了（死循环防护）
        current_index += 1
    ...
    if fail_count < max_retries:
        ...
        # 重试，但不增加 current_step_index（原地重试）
        state["is_complete"] = False
        return state
    else:
...
def summarizer_node(state: WorkflowState) -> WorkflowState:
    ...
    if mode == "react":
        # ReAct 模式：final_response 已在 _react_execute_full 中设置
    ...
    try:
        ...
        completion = client.chat.completions.create(
            model=state.get("_model", "deepseek-chat"),
            messages=[
...
def _route_after_executor(state: WorkflowState) -> Literal["reviewer", "summarizer"]:
    """Executor → Reviewer（Plan-and-Execute）或 → Summarizer（ReAct 模式直接汇总）"""
    if state.get("mode") == "react":
        return "summarizer"
    return "reviewer"
...
def build_geoagent_workflow(
    ...
    返回
    ----
    compiled : 编译好的 LangGraph StateGraph，可通过 .invoke() 调用
    """
    # 定义状态 Schema
...
class GeoAgentWorkflow:
    ...
    def run_stream(
        ...
        事件类型：
        - plan_start / plan_generated / plan_failed
        - step_start / step_end / step_skipped_deadloop
        ...
        initial_state: WorkflowState = {
            ...
            "stats": {"total_turns": 0, "tool_calls": 0, "errors": 0},
            "is_complete": False,
            "_event_callback": wrapped_callback,
            "_client": self.client,
            "_model": self.model,
            "max_steps": self.max_steps,
            "_max_retries": self.max_retries,
