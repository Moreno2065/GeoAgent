...
def _get_executor_model() -> str:
    """
    🌟 小手保护机制：Planner 使用 deepseek-reasoner，
    但 Executor / Summarizer / Correction 等小手节点必须使用 deepseek-chat。
    如果 ctx_model 是 reasoner，自动回落为 deepseek-chat。
    """
    model = ctx_model.get()
...
def _build_plan_prompt(user_query: str, available_tools: List[Dict]) -> str:
    ...
    # 🌟 核心修改 2：动态解析参数 Schema，让 LLM 每次都"开卷考试"
    tool_list_lines = []
    for t in available_tools[:15]:
        name = t.get("name") or t.get("tool_name", "unknown")
        ...
        for p_name, p_info in props.items():
            req_mark = "必填" if p_name in req else "可选"
            p_desc = p_info.get("description", "无说明")
            param_desc.append(f"    - `{p_name}` ({req_mark}): {p_desc}")
...
示例1 - 国内路径规划（必须用 amap）：
输入：在芜湖市规划从芜湖南站到方特的步行路径
输出：
{"steps":[
  {
    "step_id": 1,
    "action_name": "amap",
    "parameters": {"action": "direction_walking", "origin": "芜湖南站", "destination": "方特", "city": "芜湖市"},
    "input_files": [],
    "output_file": null,
    "description": "调用高德地图API计算芜湖南站到方特的步行导航路线",
    "depends_on": [],
    "expected_output": "高德步行导航路线JSON数据"
  }
]}

示例2 - 海外路径规划（用 osmnx_routing）：
输入：在巴黎规划从埃菲尔铁塔到卢浮宫的驾车路线
输出：
{"steps":[
  {
    "step_id": 1,
    "action_name": "osmnx_routing",
    "parameters": {"city_name": "Paris", "origin_address": "Eiffel Tower", "destination_address": "Louvre", "mode": "drive"},
    "input_files": [],
    "output_file": "workspace/route.html",
    "description": "计算巴黎的驾车最短路径",
    "depends_on": [],
...
{"steps":[
  {
    ...
    "reasoning": "用户需要在深圳市进行多准则选址分析（MCDA）。使用 multi_criteria_site_selection 工具，设置 criteria_weights 体现 POI 密度、道路可达性和绿地比例的权重平衡。",
...
{"steps":[
  {
    ...
    "reasoning": "用户要求计算 NDVI，第一步必须先获取遥感影像元数据（波段数、CRS、分辨率等），确保后续波段计算参数正确。input_file 直接引用用户提供的数据文件路径。",
    "parameters": {"input_file": "data/landsat.tif"},
    "input_files": ["data/landsat.tif"],
    "output_file": null,
    "depends_on": [],
    "expected_output": "影像的波段数、CRS、分辨率等元数据信息"
  },
  {
    "step_id": 2,
    "action_name": "calculate_raster_index",
    "reasoning": "第二步执行 NDVI 波段计算。使用标准 NDVI 公式 (NIR - Red) / (NIR + Red)， Landsat Band 5 为近红外，Band 4 为红光。output_file 写入 workspace 目录。",
    ...
    "depends_on": [1],
    "expected_output": "NDVI 结果栅格文件（-1 到 1 归一化值）"
  }
]}
"""
...
{"steps":[
  {
    "step_id": 1,
    "action_name": "osmnx_routing",
    "reasoning": "根据用户需求，提取城市名和起终点地址，设定出行模式（walk/drive/bike），调用 osmnx_routing 工具计算最短路径。",
...
## 输出格式
严格按以下 JSON 对象格式输出，**严禁添加任何解释、说明或 Markdown 标记以外的内容**。
每个 step 必须包含：
- `reasoning`：战略意图与参数推导过程的详细中文说明（为什么选这个工具、每个参数值是怎么推导出来的）
- `parameters`：该工具执行所需的完整参数字典（key-value 直接作为工具调用的参数）
{examples_section}
...
8. **parameters 字段必须根据工具说明，在 parameters 字典中提供该工具执行所需的完整参数字典；若该工具不需要参数则填 {{}}，禁止省略此字段**
9. **reasoning 字段必须用中文写清楚推理过程，说明每个参数值是如何从用户 query 推导出来的**
10. **`parameters` 字典中的键名和数据类型，必须绝对服从上方【可用工具】里的【参数定义】！**
11. 绝不允许自己瞎猜或编造参数名（例如工具需要 `distance`，你绝不能自作主张写成 `radius`）。
"""
...
def planner_node(state: WorkflowState) -> WorkflowState:
    """
    Planner 节点 — 调用 deepseek-reasoner 进行深度思考和规划，
    输出包含详细推理过程和精准参数的结构化 JSON 计划。
    ...
    🌟 大脑架构：Planner 强制使用 deepseek-reasoner（深度思考 + 富指令）
    """
    # 🌟 从上下文获取 client；model 强制指定为 deepseek-reasoner
    client: OpenAI = ctx_client.get()
    model_name = "deepseek-reasoner"  # 🌟 大脑专用模型

    try:
        user_query = state.get("user_query", "")

        _emit(state, "plan_start", {"msg": "🧠 大脑深度思考中，正在制定执行计划..."})

        # ── 替换 planner_node 中的工具获取逻辑 ──
        try:
            raw_tools = retrieve_gis_tools(user_query, top_k=15)
        except Exception:
            raw_tools = []

        # 🌟 核心修改 1：获取全量工具的完整 Schema（包含参数精确定义）
        all_schemas = {s["function"]["name"]: s["function"] for s in _get_tool_schemas() if "function" in s}

        tools = []
        for t in raw_tools:
            t_name = t.get("tool_name", "")
            if t_name in all_schemas:
                tools.append(all_schemas[t_name])

        # 兜底：如果没有检索到，传入所有可用工具的完整 Schema
        if not tools:
            tools = list(all_schemas.values())

        planning_prompt = _build_plan_prompt(user_query, tools)

        # ── 构建系统提示词（Rich-Context Instruction） ─────────────────────────
...
### 🌍 国内数据铁律 (CRITICAL)：高德定位 + OSM计算 双剑合璧
对于中国大陆的地址、POI（如"方特"、"芜湖南站"、"天安门"），必须严格执行以下两步策略：
1. **精准定位**：必须先调用 `amap` 工具（使用 `"action": "geocode"` 或 `"poi_text_search"`）获取这些地点的精确经纬度坐标 (lon, lat)。绝不允许直接将中文俗名传给 osmnx！
2. **空间计算与绘图**：拿到精确经纬度后，必须调用 `run_python_code` 工具，编写 Python 代码（利用 `osmnx` 下载路网、`geopandas` 生成缓冲区、`folium` 渲染网页地图等），以这些坐标为基准来执行真正的空间操作并保存结果。
"""
        ...
        # 方案三：动态注入 Few-Shot Examples（含 reasoning）
        if any(kw in query_lower for kw in ["路径", "route", "步行", "导航", "最短", "drive", "walk"]):
            examples = """
## 【One-Shot 参考】（必须严格按此格式输出）

输入：在芜湖市规划从芜湖南站到方特的步行路线并显示在地图上
输出：
{"steps":[
  {
    "step_id": 1,
    "action_name": "amap",
    "reasoning": "国内模糊地名'芜湖南站' OSM无法识别，必须先调用 amap 进行地理编码获取精确经纬度。",
    "parameters": {"action": "geocode", "address": "芜湖南站", "city": "芜湖市"},
    "input_files": [],
    "output_file": null,
    "depends_on": [],
    "expected_output": "芜湖南站的精确经纬度 (lon, lat)"
  },
  {
    "step_id": 2,
    "action_name": "amap",
    "reasoning": "同样，必须调用 amap 获取'方特'的精确经纬度。",
    "parameters": {"action": "geocode", "address": "方特", "city": "芜湖市"},
    "input_files": [],
    "output_file": null,
    "depends_on": [],
    "expected_output": "方特的精确经纬度 (lon, lat)"
  },
  {
    "step_id": 3,
    "action_name": "run_python_code",
    "reasoning": "拿到两点的精确经纬度后，使用 python 代码调用 osmnx.graph_from_point 下载芜湖市路网，通过 nearest_nodes 匹配坐标并计算最短路径，最后用 folium 绘制出路线地图并保存。",
    "parameters": {"code": "import osmnx as ox\\nimport networkx as nx\\nimport folium\\n# LLM根据上两步结果编写具体寻路和绘图代码..."},
    "input_files": [],
    "output_file": "workspace/wuhu_route.html",
    "depends_on": [1, 2],
    "expected_output": "包含最短路径折线的交互式 Folium 地图文件"
  }
]}
"""
...
{"steps":[
  {
    "step_id": 1,
    "action_name": "multi_criteria_site_selection",
    "reasoning": "用户需要在深圳市进行多准则决策分析（MCDA）进行智能选址。使用 multi_criteria_site_selection 工具，criteria_weights 体现 POI 密度（权重 0.4）、道路可达性（0.3）和绿地比例（0.3）的综合权衡。",
    "parameters": {"city_name": "深圳市", "criteria_weights": {"POI_density": 0.4, "road_accessibility": 0.3, "green_space_ratio": 0.3}},
    "input_files": [],
    "output_file": "workspace/site.html",
    "depends_on": [],
    "expected_output": "选址结果 GeoDataFrame 和可视化地图"
  }
]}
...
{"steps":[
  {
    "step_id": 1,
    "action_name": "get_raster_metadata",
    "reasoning": "用户要求计算 NDVI，第一步必须先获取遥感影像元数据（波段数、CRS、分辨率），确保后续波段计算参数正确。input_file 直接引用用户提供的数据文件路径。",
    "parameters": {"input_file": "data/satellite.tif"},
    "input_files": ["data/satellite.tif"],
    "output_file": null,
    "depends_on": [],
    "expected_output": "影像的波段数、CRS、分辨率等元数据信息"
  },
  {
    "step_id": 2,
    "action_name": "calculate_raster_index",
    "reasoning": "第二步执行 NDVI 波段计算。使用标准 NDVI 公式 (NIR - Red) / (NIR + Red)。对于 Landsat，Band 5 为近红外，Band 4 为红光波段，band_math_expr 设为 '(B5-B4)/(B5+B4)'。output_file 写入 workspace 目录。",
    "parameters": {"input_file": "data/satellite.tif", "output_file": "workspace/ndvi.tif", "band_math_expr": "(B5-B4)/(B5+B4)"},
    "input_files": ["data/satellite.tif"],
    "output_file": "workspace/ndvi.tif",
    "depends_on": [1],
    "expected_output": "NDVI 结果栅格文件（值域 -1 到 1）"
  }
]}
"""
...
{"steps":[
  {
    "step_id": 1,
    "action_name": "render_3d_map",
    "reasoning": "用户需要生成上海市建筑物 3D 可视化地图。使用 render_3d_map 工具，input_file 引用工作区中的建筑物矢量数据文件，渲染模式使用默认的 pydeck_3d。",
    "parameters": {"city_name": "上海市"},
    "input_files": ["data/buildings.shp"],
    "output_file": "workspace/3d_map.html",
    "depends_on": [],
    "expected_output": "PyDeck 3D 交互地图 HTML 文件"
  }
]}
"""
        else:
...
{"steps":[
  {
    "step_id": 1,
    "action_name": "amap",
    "reasoning": "天安门是国内地名，需先用 amap 查出精确坐标。",
    "parameters": {"action": "geocode", "address": "天安门", "city": "北京市"},
    "input_files": [],
    "output_file": null,
    "depends_on": [],
    "expected_output": "天安门的精确经纬度 (lon, lat)"
  },
  {
    "step_id": 2,
    "action_name": "run_python_code",
    "reasoning": "利用高德返回的天安门经纬度，使用 geopandas 创建点要素，转换为投影坐标系(如 EPSG:3857)后执行 500m buffer 操作，最后存为文件或用 folium 渲染。",
    "parameters": {"code": "import geopandas as gpd\\nfrom shapely.geometry import Point\\nimport folium\\n# LLM根据第一步结果编写缓冲区代码..."},
    "input_files": [],
    "output_file": "workspace/tiananmen_buffer.html",
    "depends_on": [1],
    "expected_output": "缓冲区地图文件"
  }
]}
"""
        ...
        # ── 主路径：直接调用 deepseek-reasoner（流式思考 + JSON 输出） ─────────
        # 🌟 reasoner 模型不支持 response_format / structured_output，
        #    必须使用原始 client.chat.completions.create 并解析输出
        try:
            resp = client.chat.completions.create(
                ...
                temperature=0.1,
                max_tokens=4096,
                timeout=120.0,  # 🌟 深度思考需要更长时间
                # 🌟 禁止：response_format / tools / structured_output
            )
            raw = resp.choices[0].message.content or ""
            print(f"[planner_node] deepseek-reasoner 原始输出 -> {raw[:500]}{'...' if len(raw) > 500 else ''}")
            ...
            if not plan:
                ...
                strict_system = (
                    ...
                    "每个 step 必须包含 reasoning（推理过程）和 parameters（参数字典）。"
                    "示例：`{\"steps\": [{\"step_id\":1,\"action_name\":\"osmnx_routing\",\"reasoning\":\"用户需要步行路径\",\"parameters\":{\"city_name\":\"北京市\",\"origin_address\":\"天安门\",\"destination_address\":\"北京站\",\"mode\":\"walk\"},\"input_files\":[],\"output_file\":\"workspace/route.html\",\"depends_on\":[],\"expected_output\":\"路径和地图\"}]}`"
                )
                strict_user = (
            ...
            print(f"[planner_node] API 调用异常 -> {api_err}")
            plan = []
...
def _execute_single_step(
    ...
    🌟 小手架构：直接使用 Planner (deepseek-reasoner) 输出的精确 parameters 字典，
       废弃原有的正则推断逻辑。保留 input_files / output_file 补全逻辑作为兜底。
    """
    plan = state.get("plan", [])
    ...
    # ── 🌟 直接提取 Planner 输出的 parameters（废弃正则推断） ───────────────
    arguments: Dict[str, Any] = step.get("parameters", {})
    if not arguments:
        arguments = {}

    # 兜底：将 input_files 的第一个文件补入 input_file
    input_files = step.get("input_files", [])
    if isinstance(input_files, list):
        for f in input_files:
            if f and f not in ("null", "", None) and "input_file" not in arguments:
                arguments["input_file"] = f
                break

    # 兜底：将 output_file 补入 arguments（部分工具需要）
    output_file = step.get("output_file")
    if output_file and output_file not in ("null", "") and "output_file" not in arguments:
        arguments["output_file"] = output_file

    # 注入 user_query（供需要城市/地址上下文的基础工具使用）
    user_query = state.get("user_query", "")
...
def _react_execute_full(state: WorkflowState) -> WorkflowState:
    ...
    🌟 小手架构：强制使用 deepseek-chat 进行工具调用。
    """
    # 🌟 从上下文获取 client；model 强制使用 deepseek-chat（小手保护）
    client: OpenAI = ctx_client.get()
    model_name = _get_executor_model()
    ...
    for turn in range(max_steps):
        ...
        for tc in tool_calls:
            ...
            try:
                ...
                success = tr_data.get("success", True)
                # 从 run_python_code 等工具提取 traceback 信息
                error_type = tr_data.get("error_type")
                error_summary = tr_data.get("error_summary")
                stderr_output = tr_data.get("stderr")
            except Exception:
                success = True
...
class GeoAgentWorkflow:
    ...
    def run(
        ...
        # 🌟 修复：必须在 run 方法里也注入上下文！
        ctx_client.set(self.client)
        ctx_model.set(self.model)
