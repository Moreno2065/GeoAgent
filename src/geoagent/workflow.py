...
{"steps":[
  {
    ...
    "reasoning": "用户需要从上海市徐家汇步行到外滩，要求最短步行路径。origin_address 和 destination_address 直接取自用户输入，mode 设为 walk（步行模式）。",
...
{"steps":[
  {
    "step_id": 1,
    "action_name": "osmnx_routing",
    "reasoning": "用户需要从天安门驾车到北京站，属于城市间驾车导航。mode 应设为 drive（驾车模式），城市名为北京市。",
    "parameters": {"city_name": "北京市", "origin_address": "天安门", "destination_address": "北京站", "mode": "drive"},
    "input_files": [],
    "output_file": "workspace/route.html",
    "depends_on": [],
    "expected_output": "包含驾车路线折线的交互式 Folium 地图文件"
  }
]}
"""
...
### 🌍 国内数据铁律 (CRITICAL)
中国大陆的地址、POI 搜索和路径规划，优先使用 `amap` 工具！
绝不可对国内非著名地名（如"方特"、"芜湖南站"、"某某小区"）使用 `osmnx_routing`，因为其底层的 OpenStreetMap 数据库无法识别国内俗名，必定导致任务彻底失败！
"""
...
{"steps":[
  {
    "step_id": 1,
    "action_name": "osmnx_routing",
    "reasoning": "用户需要从北京市天安门步行到北京站，属于城市内步行导航。city_name 取'北京市'，origin_address 和 destination_address 直接从用户 query 提取，mode 设为 'walk'。避开高速公路，因步行优先。",
    "parameters": {"city_name": "北京市", "origin_address": "天安门", "destination_address": "北京站", "mode": "walk"},
    "input_files": [],
    "output_file": "workspace/route.html",
    "depends_on": [],
    "expected_output": "包含最短路径折线的交互式 Folium 地图文件"
