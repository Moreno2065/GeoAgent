...
class ParameterExtractor:
    ...
    def extract_all(self, query: str, scenario: "str | Scenario | None" = None) -> Dict[str, Any]:
        ...
        # ── multi_criteria_search 场景 ──────────────────────────────
            ...
            # 解析多条件搜索参数
            search_params = self._parse_search_criteria(query)
            params.update(search_params)
            # 如果没有明确中心位置，从locations中获取
    ...
    def _parse_search_criteria(self, query: str) -> Dict[str, Any]:
        ...
        criteria: Dict[str, Any] = {
            "center": None,
            "radius": 3000,  # 默认3公里
            "poi_types": [],  # 要搜索的 POI 类型
            "distance_conditions": [],  # 距离条件列表
        }

        # 1. 提取中心位置
        center_patterns = [
            r"(?:在|找|搜索|分析|以)\s*([^\s,，。]+?\s*(?:附近|周围|周边|以内|以内|范围))",
            r"([^\s,，。]+?)\s*(?:附近|周围|周边)\s*(?:的|找|搜索|分析|以内|范围内)",
            r"(?:以|从)\s*([^\s，。]+?)\s*(?:为中心|起点|基准)",
        ]
        for pattern in center_patterns:
            match = re.search(pattern, query)
            if match:
                criteria["center"] = match.group(1).strip()
                break

        # 2. 提取搜索半径
        radius_patterns = [
            (r"(\d+(?:\.\d+)?)\s*公里\s*(?:范围|内|半径|方圆)", 1000),
            (r"(\d+(?:\.\d+)?)\s*km\s*(?:范围|内|半径|方圆)", 1000),
            (r"(\d+(?:\.\d+)?)\s*米\s*(?:范围|内|半径|方圆)", 1),
            (r"(\d+(?:\.\d+)?)\s*m\b(?![a-zA-Z])", 1),
            (r"方圆\s*(\d+(?:\.\d+)?)\s*(?:公里|km|米|m)", 1),
        ]
        for pattern, multiplier in radius_patterns:
            match = re.search(pattern, query, re.IGNORECASE)
            if match:
                value = float(match.group(1))
                criteria["radius"] = int(value * multiplier)
                break

        # 3. 提取 POI 类型
        poi_keywords = {
            "星巴克": ["星巴克", "starbucks", "咖啡"],
            "地铁站": ["地铁站", "地铁", "metro", "subway"],
            "便利店": ["便利店", "711", "全家", "罗森"],
            "餐厅": ["餐厅", "饭店", "餐馆", "食堂"],
            "公园": ["公园", "绿地", "广场"],
            "银行": ["银行", "atm"],
        ...
        for poi_type, keywords in poi_keywords.items():
            for kw in keywords:
                if kw in query.lower():
                    found_pois.add(poi_type)
                    break
        criteria["poi_types"] = list(found_pois)

        # 4. 提取距离条件
        # 模式: 距离 [POI] [小于/大于/在] [数值] [米/公里]
        distance_patterns = [
            # "距离星巴克小于200米"
            r"距离\s*([^\s，,]+?)\s*(?:小于|小于|小于|大于|超过|不到|不超过)\s*(\d+(?:\.\d+)?)\s*(?:米|m|公里|km)",
            # "星巴克小于200米"
            r"([^\s，,]+?)\s*(?:小于|大于|不到|不超过)\s*(\d+(?:\.\d+)?)\s*(?:米|m|公里|km)",
            # "200米内的星巴克"
            r"(\d+(?:\.\d+)?)\s*(?:米|m)\s*(?:以内|之内|内|范围|内|的)\s*([^\s，。]+?)",
        ]
        for pattern in distance_patterns:
            ...
            for match in matches:
                groups = match.groups()
                if len(groups) >= 2:
                    poi_name = groups[0].strip() if len(groups) == 2 else groups[1].strip()
                    distance_str = groups[1].strip() if len(groups) == 2 else groups[0].strip()

                    # 判断是"小于"还是"大于"
                    is_less = any(kw in match.group() for kw in ["小于", "不到", "不超过", "<", "以内", "之内"])
                    is_greater = any(kw in match.group() for kw in ["大于", "超过", ">", "以外"])

                    # 解析距离数值
                    dist_match = re.search(r"(\d+(?:\.\d+)?)\s*(?:米|m|公里|km)", distance_str)
                    if dist_match:
                        ...
                        condition = {
                            ...
                            "operator": "<" if is_less else (">" if is_greater else "<"),
                        }
                        criteria["distance_conditions"].append(condition)

        # 5. 智能推断：如果没有明确指定POI类型，但从条件中可以推断
        if not criteria["poi_types"] and criteria["distance_conditions"]:
            inferred_types = set()
            for cond in criteria["distance_conditions"]:
                poi = cond.get("poi_type", "")
                if "星巴克" in poi or "咖啡" in poi:
                    inferred_types.add("星巴克")
                if "地铁" in poi:
                    inferred_types.add("地铁站")
            criteria["poi_types"] = list(inferred_types)
...
class ScenarioOrchestrator:
    ...
    def _build_task(
        ...
        elif scenario == "multi_criteria_search":
            # 从自然语言中提取多条件搜索参数
            inputs["center"] = params.get("center", params.get("location", ""))
            inputs["user_input"] = user_input  # 保留原始输入供执行器解析
            # 解析搜索条件
            criteria = self._parse_search_criteria(user_input)
            parameters["criteria"] = criteria
            parameters["search_radius"] = params.get("search_radius", 3000)
