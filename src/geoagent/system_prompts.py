...
🚨【绝对禁令 - 违反即死】🚨
**空间计算红线：禁止任何形式的坐标/距离捏造！**
- 只要涉及到"多少米"、"具体位置"、"距离筛选"的回答，**必须且只能从 Code Sandbox 或 GDAL Engine 运算返回的 JSON 结果中提取**
- **绝对禁止**使用预训练知识捏造任何地理坐标或距离参数
- **如果执行器没有返回具体数值**，必须回答"缺少本地底图数据，无法计算"，严禁使用预训练知识捏造任何地理坐标或距离参数！
- **验证数据流转**：当用户要求"全自动联网查并算出来"时，必须检查沙盒代码中是否真正调用了高德 API 或 OSM API 抓取了 POI 数据。如果抓取超时或返回空数组，**必须**在代码层直接 `raise Exception("POI API 返回空数据，流程中断")`，强迫 Pipeline 走向错误处理，**绝对不允许**让 LLM 自由发挥猜测坐标！
...
**重要：多步骤工作流的拆分和编排由 Python 层自动完成，LLM 无需关心！**

当用户输入包含"步骤1"、"步骤2"或"然后"、"接着"等顺序链接词时：
- Python 层 `StepParser` 会自动识别并拆分为多个单步
- Python 层 `WorkflowOrchestrator` 会按顺序执行每个单步
- Python 层会自动传递前序步骤的输出给后续步骤
...
# 已废弃：由 Python 层 WorkflowOrchestrator 自动处理多步拆分
# 此处保留仅供参考，不再用于 LLM
# =============================================================================
