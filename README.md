...
│  LLM 翻译 NL → GeoDSL JSON（DeepSeek / GLM 多模型支持） │
...
| **Reasoner 翻译层** | Layer 4 使用 LLM 将 NL 翻译为 GeoDSL JSON，支持 DeepSeek/GLM 双模型 |
...
GeoAgent 使用 LLM 进行自然语言理解，支持 DeepSeek 和智谱 GLM 双模型：
...
# 智谱 GLM（可选）
echo ZHIPU_API_KEY="your-zhipu-api-key" > .env
...
基于 LLM 的自然语言 → GeoDSL 翻译器，支持 DeepSeek 和 GLM 双模型：
...
| `glm-4-plus` | open.bigmodel.com | 智谱 GLM-4 Plus |
| `glm-4-flash` | open.bigmodel.com | 智谱 GLM-4 Flash |
...
│   ├── reasoner.py        # GeoAgentReasoner（LLM NL→GeoDSL 翻译，支持 DeepSeek/GLM）
...
| `ZHIPU_API_KEY` | 否 | 智谱 GLM API 密钥（可选的 LLM 模型） |
