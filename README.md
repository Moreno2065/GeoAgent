...
![Version](https://img.shields.io/badge/Version-0.2.0-2ecc71)
![License](https://img.shields.io/badge/License-MIT-orange.svg)

**GeoAgent** 是一个基于 DeepSeek API 的空间智能 GIS 分析 Agent 系统。用户通过自然语言（中英文皆可）描述分析需求，系统将其转化为结构化任务并调用确定性后端引擎执行。覆盖路径规划、缓冲区分析、叠加分析、插值计算、阴影分析、NDVI 计算、热点分析、可视化等八大场景。

> 核心理念：**LLM 仅负责翻译（自然语言 → DSL 参数），后端代码负责路由和执行，无 ReAct 推理循环**。
...
GeoAgent 采用**三层收敛（Three-Layer Convergence）**架构，确保分析过程可预测、可测试、可解释：
...
└─────────────────────────┬───────────────────────────────┘
                          ▼
┌─────────────────────────────────────────────────────────┐
│  Layer 4 · DSL 层   DSLBuilder                          │
│  GeoDSL 结构化协议（Pydantic 模型验证）                   │
│  scenario / inputs / parameters / outputs                │
└─────────────────────────┬───────────────────────────────┘
                          ▼
┌─────────────────────────────────────────────────────────┐
...
| **无 ReAct 循环** | LLM 仅做 NL → DSL 参数翻译，后端代码负责路由，不做工具选择 |
...
GeoAgent 使用 DeepSeek API 进行自然语言理解，需要配置 API 密钥：
